...
function getTerrainBlocks(x, z) {


    const biomeValue = biome(
        x,
        z,
        terrainSeed,
        terrainRadius
...
function generateTerrain(
    ...
    // COMPUTE THE BOUNDS AS LISTS [X, Z]
    terrainMin = [
        terrainCenterX - radius,
        terrainCenterZ - radius
    ];

    terrainMax = [
        terrainCenterX + radius + (terrainChunkSize - 1),
        terrainCenterZ + radius + (terrainChunkSize - 1)
    ];
...
function generateUndergroundChunk() {
    ...
    for (
        let localX = 0;
        localX < undergroundChunkSize;
        localX++
    ) {

        for (
            let localZ = 0;
            localZ < undergroundChunkSize;
            localZ++
        ) {
    ...
    if (
        ...
        generateOres(terrainMin, terrainMax, terrainBottomY);
